...
async def create(user: required_user, request: CreateProject, service: ProjectService = Depends(get_project_service)):
...
def get_project_by_id(user: required_user, id: int = Path(gt=0), service: ProjectService = Depends(get_project_service)):
...
def get_all_by_leader(user: required_user, service: ProjectService = Depends(get_project_service)):
...
def changed_status(user: required_user, id: int = Path(gt=0),
...
def deleted(user: required_user, id: int = Path(gt=0), service: ProjectService = Depends(get_project_service)):
