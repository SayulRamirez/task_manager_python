...
def create_task(user: filter_token, request: CreateTask, service: TaskService = Depends(get_task_service)):
...
def get_task(user: filter_token, id: int = Path(gt=0), service: TaskService = Depends(get_task_service)):
...
def get_tasks_by_responsible(user: filter_token, id_responsible: int = Path(gt=0), service: TaskService = Depends(get_task_service)):
...
def changed_status(user: filter_token,
                   id: int = Path(gt=0),
...
def deleted(user: filter_token, id: int = Path(gt=0), service: TaskService = Depends(get_task_service)):
