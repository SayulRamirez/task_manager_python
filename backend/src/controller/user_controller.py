...
async def get_info(id: int = Path(gt=0), service: UserService = Depends(get_user_service)):
...
async def update(id: int = Path(gt=0), request: UpdateUser = Body(), service: UserService = Depends(get_user_service)):
